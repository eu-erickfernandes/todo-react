import styles from './ItemTarefa.module.css'
import './assets/delete.svg'

const ItemTarefa = ({ id, titulo, onClick }) => {
    const handleClick = () => {
        onClick(id)
    }

    return(
        <li className={ styles.itemTarefa }>
            <div className={ styles.tarefa__containerTitulo }>
                <input id={ `tarefa-${id}` } type='checkbox'/>
                <label htmlFor={ `tarefa-${id}` }>{ titulo }</label>
            </div>

            <button onClick={ handleClick } className={ `${ styles.tarefa__botao } ${ styles.apagar }` } title='Apagar tarefa'>
                <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24"><path d="M280-120q-33 0-56.5-23.5T200-200v-520q-17 0-28.5-11.5T160-760q0-17 11.5-28.5T200-800h160q0-17 11.5-28.5T400-840h160q17 0 28.5 11.5T600-800h160q17 0 28.5 11.5T800-760q0 17-11.5 28.5T760-720v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z"/></svg>
            </button>
        </li>
    )
}

export default ItemTarefa